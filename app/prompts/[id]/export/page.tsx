...
export default function PromptExportPage() {
  ...
  const load = useCallback(async () => { try { const result = await promptApi<{ prompt: PromptDetail }>(`/api/prompts/${params.id}`); setPrompt(result.prompt) } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to load this prompt.') } finally { setLoading(false) } }, [params.id])
  useEffect(() => { load() }, [load])
  ...
  if (!prompt) return <div className="min-h-screen bg-[#0A0A10] p-8 text-center text-[#F2F2F6]">{error || 'Prompt not found.'}</div>
