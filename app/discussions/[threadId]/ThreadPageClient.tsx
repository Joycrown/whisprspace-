...
const ThreadPage = () => {
  ...
  const { hasOlder, isLoadingOlder, loadOlder } = useOlderThreadMessages(threadId ?? undefined, session.user?.id);
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    if (now - lastMarkedReadRef.current < 30_000) return;
    lastMarkedReadRef.current = now;
    void markThreadRead(threadId);
  }, [threadId, currentThread?.hasJoined, latestMessageId]);
  ...
  const isSelfParticipant = Boolean(
    session.user?.id && currentThread?.participants?.some((participant: { id: string }) => participant.id === session.user?.id)
  );
  ...
  }, [threadId, session.user?.id, isSelfParticipant, banPresentation]);
