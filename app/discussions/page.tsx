...
const ThreadsPage = () => {
  ...
  const { newThreadCount, showNewThreads } = useRealtimeFeed(process.env.NODE_ENV === 'production');
      ...
      <div className="flex-1 min-w-0 border-x-0 md:border-x border-gray-800 flex flex-col relative overflow-x-hidden max-w-full">
        ...
        <div className="pt-2 md:pt-4 border-b border-gray-800 flex-shrink-0 w-full overflow-hidden">
          <div className="flex gap-1 md:gap-4 overflow-x-auto scrollbar-hide px-3 md:px-4 pb-0">
            ...
            <Link
              href="/my-discussions"
              className="ml-auto pb-2 md:pb-3 px-2 md:px-4 font-semibold text-xs md:text-base whitespace-nowrap border-b-2 border-transparent text-gray-500 hover:text-gray-300 flex-shrink-0"
            >
              My Discussions
            </Link>
          </div>
                ...
                {newThreadCount > 0 && (
                  <div className="sticky top-2 z-20 flex justify-center py-2">
                    <button
                      ...
                      className="rounded-full bg-purple-600 px-4 py-1.5 text-xs font-semibold text-white shadow-lg shadow-purple-900/40 active:scale-95"
                    >
                      {newThreadCount === 1 ? '1 new discussion' : `${newThreadCount > 20 ? '20+' : newThreadCount} new discussions`}
                    </button>
                  </div>
                )}
