...
export async function POST(request: Request) {
  try {
    const { email, inviteUrl, threadTitle } = await request.json();
    ...
    const subject = `You\u2019re invited to "${threadTitle || 'a thread'}"`;
        ...
        <tr>
          <td class="content">
            ...
            <p style="margin: 0 0 18px; line-height: 1.6; font-size: 16px; color: #4b5563;">
              You have been invited to join "${threadTitle || 'a private thread'}".
            </p>
            <div class="button-container">
              <a href="${inviteUrl}" class="button">Join Thread</a>
            </div>
