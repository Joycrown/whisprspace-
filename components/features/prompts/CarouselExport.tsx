...
export default function CarouselExport({ source }: CarouselExportProps) {
  ...
  const results = useMemo(() => {
    if (!isChoice || selected.length) return null
  ...
  }, [isChoice, selected.length, source.choice, source.responses])
  const slides = useMemo<ExportSlide[]>(() => {
    if (results) {
      return [{ kind: 'cover' }, { kind: 'results', entries: results.entries, headline: results.headline }, { kind: 'cta', url: source.url, cta: source.finalCta }]
    }
  ...
  }, [results, selected, ratio, maxReplies, source.url, source.finalCta])
