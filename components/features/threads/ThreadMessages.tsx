...
const MessageItem: React.FC<{
  ...
  const handleSaveEdit = () => {
    const nextValue = editContent.trim();
    if (!nextValue) {
      setEditContent(message.content);
      setIsEditing(false);
      return;
    }
    if (nextValue !== message.content) {
      onEditMessage?.(message.id, nextValue);
    }
    setIsEditing(false);
  };
        ...
        <div className="flex-1 min-w-0 space-y-2 max-w-full overflow-hidden">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-gray-500 font-medium flex items-center gap-1">
              ...
              {message.isEdited && (
                <span className="text-[10px] uppercase tracking-wide text-gray-500">(edited)</span>
              )}
          ...
          {isEditing ? (
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-md bg-purple-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-purple-700 transition-colors"
                >
                  Save
                </button>
                <button
                  ...
                  className="rounded-md border border-gray-700 px-3 py-1.5 text-xs text-gray-200 hover:bg-gray-800 transition-colors"
                >
                  Cancel
                </button>
              </div>
            ...
            <p className="text-gray-100 whitespace-pre-wrap break-words overflow-wrap-anywhere w-full text-[15px] leading-relaxed">
              {message.content}
            </p>
          )}
